...
class Reconstructor:
    ...
    def get_surface_info(self, image, ppmm):
        ...
        with torch.no_grad():
            gxyangles = self.gxy_net(features)
            gxyangles = gxyangles[0].cpu().detach().numpy()
            G = np.tan(gxyangles.transpose(1, 2, 0)) * 8
            ...
            # print("gxyangles min:", gxyangles.min(), "max:", gxyangles.max())
        ...
        if self.contact_mode == "standard":
            ...
            # color_mask = np.linalg.norm(diff_image, axis=-1)
            color_mask = np.linalg.norm(diff_image, axis=-1) > 18
            # cv2.imshow("color_mask", color_mask.astype(np.uint8) * 255)
            # cv2.waitKey(0)
            # cv2.destroyAllWindows()
            # color_mask = cv2.dilate(
            #     color_mask.astype(np.uint8), np.ones((7, 7), np.uint8))
            color_mask = cv2.dilate(
                color_mask.astype(np.uint8), np.ones((13, 13), np.uint8))
            ...
            color_mask = cv2.erode(
                color_mask.astype(np.uint8), np.ones((19, 19), np.uint8))
            # cv2.namedWindow("color_mask", cv2.WINDOW_NORMAL)
            # cv2.imshow("color_mask", color_mask.astype(np.uint8) * 255)
            # cv2.waitKey(0)
            # cv2.destroyAllWindows()
            ...
            # Filter by height
            # 计算高度图的 85 百分位值，并减去一个基于 ppmm 的偏移量
            cutoff = np.percentile(H, 60) - 0.29 / ppmm
            height_mask = H < cutoff
            # cv2.namedWindow("height_mask", cv2.WINDOW_NORMAL)
            # cv2.imshow("height_mask", height_mask.astype(np.uint8) * 255)
            # cv2.waitKey(0)
            # cv2.destroyAllWindows()
            C = np.logical_and(color_mask, height_mask)
            # cv2.namedWindow("C", cv2.WINDOW_NORMAL)
            # cv2.imshow("C", C.astype(np.uint8) * 255)
            # cv2.waitKey(0)
            # cv2.destroyAllWindows()
        elif self.contact_mode == "flat":
            # Find the contact mask based on color difference
            diff_image = image.astype(np.float32) - self.bg_image.astype(np.float32)
            color_mask = np.linalg.norm(diff_image, axis=-1) < 199
            color_mask = cv2.dilate(
                color_mask.astype(np.uint8), np.ones((15, 15), np.uint8))
            C = cv2.erode(
                color_mask.astype(np.uint8), np.ones((25, 25), np.uint8)).astype(np.bool_)
        return G, H, C
