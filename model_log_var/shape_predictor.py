...
class ShapePredictor(nn.Module):
    def __init__(self, input_dim=17, output_dim=14):
        super().__init__()
        # 1. Feature block encoder
        self.scale_encoder = self._create_scale_encoder()
        self.shape_encoder = self._create_shape_encoder()
        self.pose_encoder = self._create_pose_encoder()
        self.geometry_encoder = self._create_geometry_encoder()
        self.control_encoder = self._create_control_encoder()

        # 2. Cross-attention mechanism - capture the interaction between features
        self.cross_attention = CrossAttentionModule(
            feature_dims=[32, 16, 24, 24, 16]  # output dimension of each encoder
        )

        # 3. Feature fusion network
        self.fusion_network = FusionNetwork(
            input_dim=160,  # total dimension after cross attention
    ...
    def _initialize_physics_aware(self):
        """Initialize all weights with decimal values."""
    ...
    def _create_pose_encoder(self):
        class EulerToQuatLayer(nn.Module):
            ...
            def forward(self, x):
                ...
                half_angles = euler / 2.0

                cx, sx = torch.cos(half_angles[:, 0]), torch.sin(half_angles[:, 0])
                cy, sy = torch.cos(half_angles[:, 1]), torch.sin(half_angles[:, 1])
                cz, sz = torch.cos(half_angles[:, 2]), torch.sin(half_angles[:, 2])
    ...
    def forward(self, current_state, control):
        ...
        pose_feat = self.pose_encoder(torch.cat([
            current_state[:, 5:8],
            current_state[:, 8:11]
        ], dim=1))
        ...
        delta_state = self.delta_head(fused_features)
        uncertainty = self.uncertainty_head(fused_features)

        return delta_state, uncertainty
