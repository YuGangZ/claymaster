...
def plot_predictions_vs_truth(model, dataloader, save_dir='training_results', num_samples=1000):
    ...
    fig.suptitle('Predictions vs Ground Truth Analysis', fontsize=16, y=0.98)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'predictions_vs_truth_detailed.png'), dpi=300, bbox_inches='tight')
    plt.close()

    # 额外：绘制delta预测质量
    fig_delta = plt.figure(figsize=(15, 10))
    fig_delta.suptitle('Delta Prediction Quality', fontsize=16, y=0.98)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'delta_prediction_quality.png'), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    if len(all_preds) > 0:
        ...
        plot_uncertainty_calibration(
            all_preds, all_uncertainties, all_targets,
            os.path.join(save_dir, 'uncertainty_calibration.png')
        )
